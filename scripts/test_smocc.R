...
full <- smocc[[2]] %>% 
  select(id, rec, age, sex, ga, bw, 
         hgt, wgt, hdc, bmi, 
         hgt.z, wgt.z, hdc.z, bmi.z) %>% 
  left_join(select(smocc[[1]], id, edu, twin, agem, smo, hgtm, hgtf, hdc_0), 
            by = c("id")) %>% 
  select(id, rec, age, sex, ga, bw, edu:hgtf, hgt:bmi.z, hdc_0)
