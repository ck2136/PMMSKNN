...
#' @param ord_data Data frame. Specifically, training data with id ordered based on fitted distal outcome value using predicted mean matching.
...
matchTrainDataGen <- function(
    ...
    if(matchprobweight){
        ...
        colnames(matchindexdf)[1] = idname
        matchindexdf %>% 
            left_join(
                matchprob,
                by = idname
        ...
        matches <- matchindexdf[unique(matchindex),idname]
        matchmodel <- train_post[train_post[[idname]] %in% matches, ]
        return(matchmodel)
    } else {
        matches <- matchIdExtract(ord_data, mtype, loocv=loocv, n,m, i) 
        matchmodel <- train_post[train_post[[idname]] %in% matches, ]
